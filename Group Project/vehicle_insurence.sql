...
-- query 1
    ...
    where c.Cust_id in ( 
        ...
        call Getcustomerandvehcile('pending');
...
create view query_1 as
    select v.Vehicle_Id, v.Policy_Id, v.Dependent_NOK_Id, v.Vehicle_Registration_Number, v.Vehicle_Value, v.Vehicle_Type, v.Vehicle_Size, v.Vehicle_Number_Of_Seat, v.Vehicle_Manufacturer, v.Vehicle_Engine_Number, v.Vehicle_Chasis_Number, v.Vehicle_Number, v.Vehicle_Model_Number, v.Cust_Id, c.Cust_FName, c.Cust_LName, c.Cust_DOB, c.Cust_Gender, c.Cust_Address, c.Cust_MOB_Number, c.Cust_Email, c.Cust_Passport_Number, c.Cust_Marital_Status, c.Cust_PPS_Number
    from  T8_VEHICLE v inner join T8_CUSTOMER c on v.Cust_id=c.Cust_id  
    where c.Cust_id in ( 
		select x.Cust_id 
        from T8_CLAIM x
...
-- query 2
    ...
    where c1.Cust_id in (
		select p.Cust_id 
        from T8_PREMIUM_PAYMENT p 
		where (p.Premium_Payment_Amount > (select sum(CAST(Cust_id as unsigned)) from T8_CUSTOMER)));

   
-- view query 2
create view query_2 as
    select c.Cust_Id, c.Cust_FName, c.Cust_LName, c.Cust_DOB, c.Cust_Gender, c.Cust_Address, c.Cust_MOB_Number, c.Cust_Email, c.Cust_Passport_Number, c.Cust_Marital_Status, c.Cust_PPS_Number
    from T8_CUSTOMER c 
    where c.Cust_id in (
		select p.Cust_id 
        from T8_PREMIUM_PAYMENT p 
...
select c1.Cust_id 
        from T8_CUSTOMER c1, T8_PREMIUM_PAYMENT p 
		where (p.Cust_id = c1.Cust_id and p.Premium_Payment_Amount > (select sum(CAST(Cust_id as unsigned)) from T8_CUSTOMER));
drop procedure cust_id;
